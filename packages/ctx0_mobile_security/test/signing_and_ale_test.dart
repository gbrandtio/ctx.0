...
final _testConfig = CtxSecurityConfig(
  resolveUri: (path) => Uri.parse('https://api.example.com/v1$path'),
  refreshTokenPath: '/users/refresh',
  authEndpointPaths: const ['/users/authenticate', '/users/refresh'],
  securityMetadataPath: '/security/metadata',
  appInstancesPath: '/security/app-instances',
);
...
void main() {
  group('SecureDeviceSigningClient', () {
    ...
    test(
      ...
      expect(registration.headers[CtxSecurityConfig.defaultDeviceIdHeader],
          identity.deviceId);
    });
  });
