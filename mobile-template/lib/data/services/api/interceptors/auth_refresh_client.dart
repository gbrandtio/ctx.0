...
class AuthRefreshClient extends http.BaseClient {
  AuthRefreshClient(this._inner, this._secureStorage, {this.onSessionExpired});
  ...
  bool _isAuthEndpoint(Uri url) {
    ...
    return path.endsWith(ApiConstants.login) ||
        path.endsWith(ApiConstants.refreshToken) ||
        path.endsWith(ApiConstants.logout) ||
        path.endsWith(ApiConstants.googleSignIn);
  }
  ...
  Future<bool> _refreshTokens() async {
    ...
    try {
      final request = http.Request(
        'POST',
        ApiConstants.uri(ApiConstants.refreshToken),
      )
