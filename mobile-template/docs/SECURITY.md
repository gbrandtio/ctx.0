...
- **Application Layer Encryption (ALE)**:
    ...
    - **Implementation**: `AleClient` interceptor (`lib/data/services/api/interceptors/ale_client.dart`) and `SecurityMetadataService` (`lib/data/services/api/security_metadata_service.dart`).
...
- **Request Signing**:
    ...
    - **Implementation**: `SecureDeviceSigningClient` interceptor (`lib/data/services/api/interceptors/secure_device_signing_client.dart`).
...
- **Runtime Application Self Protection (RASP)**:
    ...
    - **Implementation**: `RaspService` (`lib/data/services/security/rasp_service.dart`), initialized at app startup.
...
1. **RASP Check**: `RaspService` starts monitoring the environment. If the device is rooted or a debugger is attached, the app terminates immediately.
