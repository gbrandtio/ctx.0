...
/// </summary>
public sealed class RlsInterceptor(ICurrentUserProvider currentUser) : DbCommandInterceptor
{
    ...
    private DbCommand? CreateRlsCommand(DbCommand command)
    {
        ...
        if (currentUser.IsSystemBypassActive)
        {
            ...
            rlsCmd.CommandText =
                $"SELECT set_config('app.current_user_id', @userId, {isLocal}); " +
                "SET LOCAL ROLE app_internal_worker;";
        }
        else
        {
            rlsCmd.CommandText =
    ...
    private static bool IsRlsCommand(DbCommand command) =>
        command.CommandText.StartsWith("SELECT set_config('app.current_user_id'",
