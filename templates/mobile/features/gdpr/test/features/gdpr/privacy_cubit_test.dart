...
class FakeExportRepository implements PrivacyRepository {
  ...
  Future<({ExportJob job, String downloadToken})> requestExport() async {
    ...
    return (job: const ExportJob(jobId: 'job-1', status: 'Pending'), downloadToken: tokenIssued!);
  }
...
void main() {
  test('downloadMyData polls until ready, then saves the archive', () async {
    ...
    final cubit = PrivacyCubit(api, pollInterval: Duration.zero, saveArchive: sink.call);
    ...
    expect(api.tokenPresented, api.tokenIssued, reason: 'the one-time token from the request is what downloads it');
    expect(utf8.decode(sink.saved['ctx-export-job-1.zip']!), 'archive-bytes');
  });

  test('an export that never becomes ready fails rather than hanging', () async {
  ...
  blocTest<PrivacyCubit, PrivacyState>(
    ...
    build: () => PrivacyCubit(FakeExportRepository(), saveArchive: ArchiveSink().call),
  ...
  blocTest<PrivacyCubit, PrivacyState>(
    ...
    build: () => PrivacyCubit(FakeExportRepository(deleteFails: true), saveArchive: ArchiveSink().call),
