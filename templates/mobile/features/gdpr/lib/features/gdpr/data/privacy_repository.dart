...
class ConsentStatus {
  ...
  bool get needsDecision =>
      recordedVersion == null || recordedVersion != policyVersion;

  factory ConsentStatus.fromJson(Map<String, dynamic> json) {
    final consent = json['consent'] as Map<String, dynamic>?;
    return ConsentStatus(
      policyVersion: json['policyVersion'] as String,
      purposes: consent == null
          ? null
          : ((consent['purposes'] as List<dynamic>?) ?? const [])
                .cast<String>()
                .toSet(),
      decidedAt: consent?['decidedAt'] == null
          ? null
          : DateTime.parse(consent!['decidedAt'] as String),
...
class HttpPrivacyRepository implements PrivacyRepository {
  ...
  Future<({ExportJob job, String downloadToken})> requestExport() async {
    final response = await _http.post(
      Uri.parse('$_baseUrl/v1/privacy/export'),
      headers: await _headers(),
    );
    final json = _decode(response);
    return (
      job: ExportJob.fromJson(json),
  ...
  Future<ExportJob> exportStatus(String jobId) async {
    final response = await _http.get(
      Uri.parse('$_baseUrl/v1/privacy/export/$jobId'),
      headers: await _headers(),
    );
  ...
  Future<Uint8List> downloadExport({
    ...
    final uri = Uri.parse(
      '$_baseUrl/v1/privacy/export/$jobId/download',
