...
* **State Management**: Bloc (`flutter_bloc` + `bloc`). See `docs/STATE_MANAGEMENT.md` for the mandatory conventions.
* **State Modeling**: `freezed` (preferred) or `equatable` for immutable Bloc states and events.
* **Navigation**: GoRouter.
* **Networking**: Http package.
* **Local DB**: Drift (SQL) or Hive/SharedPrefs (Key-Value).
* **Code Generation**: Freezed & JsonSerializable.
* **Testing**: `bloc_test` + `mocktail` for Bloc/Cubit unit tests.
* **Existing implementations or Libraries**: If there is already an existing implementation or library, use it. Use consistent implementation across the whole project.
* **UI/UX Standards**: Follow the centralized theme and design patterns documented in `docs/UI_UX_GUIDELINES.md`.

## Coding Standards
* **Strict Typing**: Never use `dynamic`. Use `Result<T>` for all data layer returns.
* **Async**: Always use `async`/`await`.
* **Widgets**: Break complex screens into small, private `StatelessWidget` components.
* **Style**: Follow the standard Dart style guide (effective dart).
* **Comments & Documentation**: Never include prompts or thinking processes in code comments or documentation. The code comments and documentation must only focus on technical details and business logic that help readers understand more. The code comments must document WHY and NOT explain the code, the code must be self-explanatory.
* **SOLID**: Always adhere to SOLID code principles (Single-Responsibility Principle, Open-Close Principle, Liskov Substitution Principle, Interface Segregation Principle, Dependency Inversion Principle).

## Date and Time Handling
* **API & Storage**: All timestamps sent to or received from the API, and all timestamps stored in local databases (Hive/Drift), must be in **UTC**.
* **UI Display**: All user-facing timestamps must be displayed in the user's **local timezone**. Use the `.toLocal()` method on the `DateTime` object before formatting it for display (e.g., `DateFormat(...).format(date.toLocal())`).
* **Midnight Integrity**: Ensure that local time conversion is used to correctly handle events occurring around midnight UTC, as the local date may differ from the UTC date.
...
Identifiers come in three flavors in Dart:
  ...
  * **lowercase\_with\_underscores** names use only lowercase letters, even for acronyms, and separate words with `_`.
...
### DO name packages, directories, and source files using lowercase\_with\_underscores
...
### DO name import prefixes using lowercase\_with\_underscores
...
> **Version Note:** Declaring non-binding wildcard variables requires a language version of at least 3.7. In earlier versions, use additional underscores (e.g., `__`, `___`) to avoid collisions.
...
Appending a name to the `library` directive is technically possible, but is a legacy feature and discouraged. Dart generates a unique tag for each library based on its path and filename. Naming libraries overrides this generated URI. Without the URI, it can be harder for tools to find the main library file in question.
...
Formatting is tedious work and is particularly time-consuming during refactoring. Fortunately, you don't have to worry about it. We provide a sophisticated automated code formatter called `dart format` that does it for you. The official whitespace-handling rules for Dart are whatever `dart format` produces.
...
When that happens, reorganize or simplify your code. Consider shortening a local variable name or hoisting out an expression into a new local variable. In other words, make the same kinds of modifications that you'd make if you were formatting the code by hand and trying to make it more readable.
...
Note that `dart format` defaults to 80 characters or fewer, though you can configure the default. It does not split long string literals to fit in 80 columns, so you have to do that manually.
  ...
  * **Exception:** When a URI or file path occurs in a comment or string (usually in an import or export), it may remain whole even if it causes the line to go over 80 characters.
