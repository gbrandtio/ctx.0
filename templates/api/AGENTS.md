...
### 2b. Scaffoldable Features (enable/disable integrations)
*   **Context:** Enabling or disabling Stripe payments, Firebase push dispatch, Google Sign-In, or the nearby-items endpoint; anything touching `ctx:` marker blocks or `.ctx/integrations.json`.
*   **Required Reading:**
    *   `docs/INTEGRATIONS.md` (binding; `ctx0` CLI only — never hand-edit markers)
...
*   **Required Reading:** 
    ...
    *   `docs/packages/Ctx0.Security*.md` — the installed security packages' own instructions (materialized by `ctx0 docs sync`; they describe exactly the installed versions)
