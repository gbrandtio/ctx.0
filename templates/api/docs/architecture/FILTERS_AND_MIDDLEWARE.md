...
### Registration — `UseAppSecurity()`
The security middlewares are never registered individually. `app.UseAppSecurity()` (`AppApi/Extensions/SecurityExtensions.cs`) wires the full pipeline in its contractual order — ALE decryption → signature verification → authentication → RLS identity propagation → authorization → rate limiting — and is the single line `Program.cs` calls. Reordering or bypassing any stage is a security defect.
