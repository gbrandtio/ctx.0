...
- RLS and envelope-encryption interceptors ship in `Ctx0.Security.EfCore`; the DbContext registers them in `ServiceCollectionExtensions`
...
#### Security — the `Ctx0.Security` packages + `Infrastructure/Security/`
The security plane ships as the compiled `Ctx0.Security`,
`Ctx0.Security.Abstractions`, and `Ctx0.Security.EfCore` packages (JWT
issuance, BCrypt hashing, envelope encryption, the RLS and encryption
interceptors, ALE + request-signing middlewares, RBAC role catalog).
Consumers pin versions — the plane is never edited or re-implemented.
`Infrastructure/Security/` keeps only the app-specific pieces:
- Resource-ownership authorization handlers (`Handlers/`)
- `AppInstanceDeviceKeyStore` — adapts the device-registration store to
  the plane's `IDeviceKeyStore`
...
- `RequestSigningMiddleware` and `AleMiddleware` ship inside the
  `Ctx0.Security` package and are wired solely by `UseAppSecurity()` →
  `UseCtxSecurity()` — never registered individually.
...
*   **Service:** Prefer injecting `IClock` from `Ctx0.Security.Abstractions` for application logic to allow for time-travel in unit tests.
