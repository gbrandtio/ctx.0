...
- **Methods**: email + password (with mandatory email verification), Google Sign-In, and optional mandatory 2FA via email (`auth_2fa_email` requiring an email provider like `email_brevo`). On mobile each method is scaffoldable (`auth_google`, `auth_email_password`, `auth_2fa_email` see: `templates/mobile/docs/INTEGRATIONS.md`). At least one stays enabled. The API always ships both endpoints; a mobile-side disable is UI-only and needs no API change.
- **Token model**: short-lived JWT access token + rotating refresh token with reuse detection and family revocation. Protocol source of truth: `templates/api/docs/security/AUTHENTICATION.md`; client mirror: `templates/mobile/docs/SECURITY.md`.
...
- **Account model notes**: <e.g., single consumer role only, or additional principal types per `templates/api/docs/security/AUTHORIZATION.md`>
