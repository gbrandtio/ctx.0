...
- **Methods**: email + password (with mandatory email verification) and Google Sign-In. On mobile each method is scaffoldable (`auth_google`, `auth_email_password` — `templates/mobile/docs/INTEGRATIONS.md`); at least one stays enabled. The API always ships both endpoints; a mobile-side disable is UI-only and needs no API change.
